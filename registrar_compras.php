                ...
                <form action="" name="frm_registrar_compras" id="frm_registrar_compras" method="POST">
                    <div class="card card-body">
                            ...
                            <select class="form-select" name="cbo_proveedores" id="cbo_proveedores">
                                ...
                                <?php
                                    ...
                                    while ($valores = mysqli_fetch_array($result)) {
                                        ?>
                                        <option value="<?php echo $valores['id_proveedor'] ?>"><?php echo $valores["nombre_proveedor"] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="col-auto mt-2 text-center">
                            <button type="button" onclick="cerrar_compra()" class="btn btn-info btn-block">Cerrar</button>
                            <button type="button" onclick="generar_compra()" class="btn btn-success btn-block">Generar</button>
                        </div>
                    </div>
                    <div class="card card-body mt-2">
                        ...
                        <div class="col-auto mt-2 text-center">
                            <button type="button" onclick="limpiar_frm_compras()" class="btn btn-info btn-block">Limpiar</button>
        ...
        function agregar_materia_prima(){
            ...
            if(confirm("¿Agregar " + cantidad.value + " " + cbo_materias_primas.selectedOptions[0].text + " a la compra?")){
                $.ajax({
                    ...
                    data: {id_compra:id_compra.value, id_materia_prima:id_materia_prima, cantidad:cantidad.value, costo:costo.value},
        ...
        function eliminar_materia_prima(id_materia_prima){
            ...
            if(confirm("¿Eliminar la materia prima del datalle?")){
                $.ajax({
                    ...
                    data: {id_compra:id_compra.value, id_materia_prima:id_materia_prima},
                    success: function(result) {
                        limpiar_frm_compras();
                        var data = $.parseJSON(result);
                        var html = '';
        ...
        function generar_compra(){
            ...
            if(confirm("¿Generar una nueva compra?")){
                $.ajax({
                    ...
                    data: {id_proveedor:id_proveedor, fecha:fecha.value},
                    success: function(result) {
                        limpiar_frm_compras();
                        var data = $.parseJSON(result);
